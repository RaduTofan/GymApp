...
const AddClient = () => {
    ...
    function handleDateChanges(date: any) {
        setStartDate(date);
    }
    ...
    useEffect(() => {
        (async () => {
            try {
                const publishers = await getAllNutritionPlans();
                setNutritionPlans(publishers);
            } catch (error) {
                console.error(error);
            }
        })()
    ...
    const onSubmit = (form: Client) => {
        (async () => {
            try {
                //let newDate = new Date(Date.parse(startDate.toString()));
                //form.dateOfBirth=`${newDate.getFullYear()}-${newDate.getMonth()}-${newDate.getDate()}`;
                console.log(form);
